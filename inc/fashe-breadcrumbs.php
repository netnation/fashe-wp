...
if ( ! function_exists( 'fashe_breadcrumbs' ) ) {
	function fashe_breadcrumbs( $args = array() ) {
		if ( is_front_page() ) {
			return;
		}
		...
		*/
		...
		if ( is_singular( 'post' ) ) {
			...
			$html .= '<span class="s-text17 bread-search">' . esc_html__( 'Search results for:', 'fashe' ) . get_search_query() . '</span>';
		} elseif ( is_404() ) {
			$html .= '<span>' . esc_html__( 'Error 404', 'fashe' ) . '</span>';
		} elseif ( is_home() ) {
			$html .= '<span class="s-text17">' . get_the_title( get_option( 'page_for_posts' ) ) . '</span>';
		}
		$html .= '</div>';
		$html  = apply_filters( 'fashe_breadcrumbs_filter', $html );
